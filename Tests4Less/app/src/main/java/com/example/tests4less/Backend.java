                    ...
                    public void onResponse(JSONArray response){
                        ...
                        try {
                            ...
                            for (int i = 0; i < response.length(); i++) {
                                ...
                                tempItem.name = item.getString(NAME);
                                tempItem.place = item.getString(PLACE);
                                tempItem.features = item.getString(FEATURES);
                                tempItem.priceActual = item.getInt(PRICEACTUAL);
                                tempItem.priceOffered = item.getInt(PRICEOFFERED);
                                tempItem.PostitionLat = item.getDouble(LATITUDE);
                                tempItem.PostionLong = item.getDouble(LONGITUDE);
                                tempItem.ratingAVG =  item.getDouble(RATINGAVG);
                                tempItem.ratingNo=  item.getInt(RATINGNO);
                                tempItem.imgURL = item.getString(IMAGEURL);
